...
export function getHorizontalAlign(node: XmlNode, pNode: XmlNode, type: string, warpObj: AnyRecord): string {
  ...
  if (!algn) {
    if (type === 'title' || type === 'ctrTitle' || type === 'subTitle') {
      ...
      if (lvlNode) {
        lvlIdx = parseInt(String(lvlNode)) + 1
      }
...
export function getVerticalAlign(node: XmlNode, slideLayoutSpNode: XmlNode, slideMasterSpNode: XmlNode): string {
...
type AutoFitResult =
  | { type: 'shape' }
  | { type: 'text'; fontScale?: number }
  | null

export function getTextAutoFit(node: XmlNode, slideLayoutSpNode: XmlNode, slideMasterSpNode: XmlNode): AutoFitResult {
  function checkBodyPr(bodyPr: XmlNode): { result: AutoFitResult } | null {
    if (!bodyPr) return null

    const body = bodyPr as AnyRecord
    if (body['a:noAutofit']) return { result: null }
    else if (body['a:spAutoFit']) return { result: { type: 'shape' } }
    else if (body['a:normAutofit']) {
      const fontScale = getTextByPathList(body['a:normAutofit'] as XmlNode, ['attrs', 'fontScale'])
      if (fontScale) {
        const scalePercent = parseInt(String(fontScale)) / 1000
        return {
          result: {
            type: 'text',
...
export function getParagraphSpacing(pNode: XmlNode): AnyRecord | null {
  if (!pNode) return null

  const pPrNode = (pNode as AnyRecord)['a:pPr'] as XmlNode
  if (!pPrNode) return null

  const spacing: AnyRecord = {}

  const lnSpcNode = (pPrNode as AnyRecord)['a:lnSpc'] as XmlNode
  if (lnSpcNode) {
    const spcPct = getTextByPathList(lnSpcNode, ['a:spcPct', 'attrs', 'val'])
    const spcPts = getTextByPathList(lnSpcNode, ['a:spcPts', 'attrs', 'val'])

    if (spcPct) {
      spacing.lineSpacing = parseInt(String(spcPct)) / 1000 / 100
    } 
    else if (spcPts) {
      spacing.lineSpacing = parseInt(String(spcPts)) / 100 + 'pt'
    }
  }

...
export function getFontType(node: AnyRecord, type: string, warpObj: AnyRecord, slideLayoutSpNode: AnyRecord, slideMasterSpNode: AnyRecord, slideMasterTextStyles: AnyRecord): string {
  ...
  if (!typeface || typeface.startsWith('+')) {
    const fontSchemeNode = getTextByPathList(warpObj['themeContent'] as AnyRecord, ['a:theme', 'a:themeElements', 'a:fontScheme'])
    ...
    if (fontSchemeNode) {
      if (typeface && typeface.startsWith('+')) {
        switch (typeface) {
          ...
          default: 
            return String(typeface).replace(/^\+/, '')
        }
      }
    }
...
export function getFontSize(node: AnyRecord, slideLayoutSpNode: AnyRecord, type: string, slideMasterTextStyles: AnyRecord, textBodyNode: AnyRecord, pNode: AnyRecord): number | null {
  ...
  if ((isNaN(fontSize) || !fontSize) && textBodyNode) {
    ...
    if (lstStyle) {
      ...
      if (sz) fontSize = parseInt(String(sz)) / 100
    }
  }
  ...
  if ((isNaN(fontSize) || !fontSize) && slideLayoutSpNode) {
    ...
    if (layoutSz) fontSize = parseInt(String(layoutSz)) / 100
  }

  if ((isNaN(fontSize) || !fontSize) && pNode) {
    const paraSz = getTextByPathList(pNode, ['a:pPr', 'a:defRPr', 'attrs', 'sz'])
    if (paraSz) fontSize = parseInt(String(paraSz)) / 100
  }

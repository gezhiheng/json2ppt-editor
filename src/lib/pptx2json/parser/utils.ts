...
export function getTextByPathList(node: XmlNode, path: Array<string>): XmlNode {
  ...
  for (const key of path) {
    ...
    current = (current as Record<string, XmlNode>)[key]
    if (!current) return current
  }
...
export function isVideoLink(vdoFile: any) {
...
export function hasValidText(htmlString: any) {
...
export function numberToFixed(num: any, fractionDigits: any = 4) {

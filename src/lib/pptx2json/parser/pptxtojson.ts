...
async function getContentTypes(zip: any) {
  const ContentTypesJson = await readXmlFile(zip, '[Content_Types].xml')
  const subObj = ContentTypesJson['Types']['Override']
  let slidesLocArray: string[] = []
  let slideLayoutsLocArray: string[] = []
  ...
  const sortSlideXml = (p1: any, p2: any) => {
    const match1 = /(\d+)\.xml/.exec(p1)
    const match2 = /(\d+)\.xml/.exec(p2)
    const n1 = match1 ? +match1[1] : 0
    const n2 = match2 ? +match2[1] : 0
    return n1 - n2
  }
...
async function getSlideInfo(zip: any) {
...
async function getTheme(zip: any) {
...
async function processSingleSlide(zip: any, sldFileName: any, themeContent: any, defaultTextStyle: any) {
  ...
  const themeResObj: Record<string, any> = {}
  const diagramResObj: Record<string, any> = {}
  ...
  if (Object.values(diagramFiles).length > 0) {
    for (const filePath of Object.values(diagramFiles) as string[]) {
...
function getNote(noteContent: any) {
  let text = ''
...
async function getLayoutElements(warpObj: any) {
  const elements = []
...
async function processNodesInSlide(nodeKey: any, nodeValue: any, nodes: any, warpObj: any, source: any, groupHierarchy: any = []) {
  let json
...
async function processMathNode(node: any, warpObj: any, source: any) {
...
async function processGroupSpNode(node: any, warpObj: any, source: any, parentGroupHierarchy: any = []) {
  const order = node['attrs']['order']
  ...
  for (const nodeKey in node) {
    if (node[nodeKey].constructor === Array) {
      for (const item of node[nodeKey]) {
        const ret: any = await processNodesInSlide(nodeKey, item, node, warpObj, source, currentGroupHierarchy)
        if (ret) elements.push(ret)
      }
    }
    else {
      const ret: any = await processNodesInSlide(nodeKey, node[nodeKey], node, warpObj, source, currentGroupHierarchy)
      if (ret) elements.push(ret)
    }
  }

  const processedElements: any[] = elements.map((element: any) => ({
    ...element,
  ...
  function processNestedGroupElements(elements: any[], ws: any, hs: any, depth: any = 0) {
...
async function processSpNode(node: any, pNode: any, warpObj: any, source: any, groupHierarchy: any = []) {
...
async function processCxnSpNode(node: any, pNode: any, warpObj: any, source: any) {
...
async function genShape(node: any, pNode: any, slideLayoutSpNode: any, slideMasterSpNode: any, name: any, type: any, order: any, warpObj: any, source: any, groupHierarchy: any = []) {
...
async function processPicNode(node: any, warpObj: any, source: any) {
  let resObj
...
async function processGraphicFrameNode(node: any, warpObj: any, source: any): Promise<any> {
...
async function genTable(node: any, warpObj: any) {
  const order = node['attrs']['order']
...
async function genChart(node: any, warpObj: any) {
  const order = node['attrs']['order']
  const xfrmNode = getTextByPathList(node, ['p:xfrm'])
  const { top, left } = getPosition(xfrmNode, undefined, undefined)
  const { width, height } = getSize(xfrmNode, undefined, undefined)
...
async function genDiagram(node: any, warpObj: any) {
  const order = node['attrs']['order']
  const xfrmNode = getTextByPathList(node, ['p:xfrm'])

...
const mapFill = (fill: any): { fill?: string; pattern?: string } => {
  if (!fill) return {};
  if (typeof fill === "string") return { fill: mapColor(fill) ?? undefined };
  if (typeof fill === "object") {
    if (fill.type === "color" && typeof fill.value === "string") {
      return { fill: mapColor(fill.value) ?? undefined };
    }
...
const buildRoundRectPath = (width: number, height: number, radius: number) =>
  `M ${radius} 0 L ${radius} 0 Q ${width} 0 ${width} ${radius} ` +
  `L ${width} ${height - radius} Q ${width} ${height} ${radius} ${height} ` +
...
const mapKeypoints = (pathFormula: string, raw: any): number[] | undefined => {
  ...
  if (pathFormula === "roundRect") return [0.5];
...
const normalizeElement = (element: SlideElement): SlideElement => {
  if (element.type === "shape" && element.height === 0 && element.width && element.outline) {
    return {
      type: "line",
      id: element.id,
  ...
  if (element.type === "shape" && element.fill) {
    element.fill = normalizeColor(element.fill) ?? element.fill;
  }
